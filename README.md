...
| `src/task-mock.html` | タスク管理（モック） | 3階層のタスク管理と日報テキストへの自動反映を試すための検証用画面。ログイン不要・保存なし。本番の日報作成画面には未反映で、ナビからの入口も無い（URL直打ちで開く） |
...
3. `supabase/` の .sql を**次の順番で**貼り付けて **Run**（1ファイルずつ、上から順に）
   ...
   ※ 手で貼るのはここまで（新しいプロジェクトを作るときの初回だけ）です。
   以降のスキーマ更新は「[データベースの更新（マイグレーション）](#データベースの更新マイグレーション)」の
   `npm run db:migrate` で流します。
...
- **新規サインアップの許可（Enable Sign Ups）はオンのままにする** … Supabase では初回の
  Google ログインが内部的には「新規アカウント作成」として扱われるため、オフにすると
  まだ一度もログインしていない社員が一切ログインできなくなる。許可ドメインの制限は
  `schema_v15_signup_allowlist.sql` の `handle_new_user()` がDB側で行っているので、
  ダッシュボード側でオフにする必要はない
- **Email の Confirm email（確認メール）はオンにしておく** … オフだと、公開JSに必ず含まれる
  匿名キーでサインアップAPIを直接叩かれ、実在しない `@teetime.co.jp` のアドレスで
  アカウントを作られる余地が残る
...
`supabase/` の .sql を SQL Editor に手で貼る運用はやめました。**まだ流していない .sql だけ**を
版番号順に流すコマンドがあります。

```bash
# 流す予定のファイルを見るだけ（DBは変えない）
...
※ `npm run db:migrate --dry-run` と書くと `--dry-run` は npm 自身のオプションとして
食われてしまい、**本当に流れます**。確認したいときは `npm run db:migrate:dry` を使ってください。

接続先は環境変数 **`SUPABASE_DB_URL`**（Postgres の接続文字列）から読みます。
値は Supabase ダッシュボードの **Project Settings > Database > Connection string** にあります。
`.env` に `SUPABASE_DB_URL=...` の行を足しておいても拾います（`.env` は Git に上げません）。
未設定のまま `npm run db:migrate` を実行した場合は、DBに触らずその旨を表示して終了します。

証明書の検証で接続できないときは、接続文字列の末尾に `?sslmode=no-verify` を付けてください。

### 仕組み

- 流したファイル名は **`public.schema_migrations`**（`filename` が主キー）に記録されます。
  次回はそこに無いものだけを流すので、同じファイルが二度流れることはありません。
  このテーブルはコマンドが自動で作ります（RLS 有効・ポリシー無しなので anon からは触れません）。
- **記録が空の初回だけは、`schema.sql` 〜 `schema_v10_knowledge.sql` を「流さずに記録だけ」します。**
  これらは本番へ適用済みで、流し直すと anon 全許可ポリシーが復活して認証が無効化されるためです
  （上の「セットアップ手順」の注意書きを参照）。
- 並び順は辞書順ではなく**版番号の数値順**です（`schema_v2.sql` → `schema_v10_knowledge.sql`）。
  `schema.sql` は v1 相当としていちばん最初に扱います。
- `supabase/migrate_user_a.sql` は**対象外**です。セットアップ用ではないため流しません。
- 1ファイルずつトランザクションで流します。途中で失敗したら、そのファイルの変更と記録は
  まとめて取り消されます。

### スキーマを変えたくなったら

- **新しいファイルを足してください**（例: `supabase/schema_v14_xxx.sql`）。
  次に `npm run db:migrate` を実行した人のDBに、自動で流れます。
- **既存の .sql は書き換えないでください。** 適用済みとして記録されているので流し直されず、
  変更が本番に反映されません。さらに古いファイルを流し直すこと自体が危険です。
- 同じSQLを二度流しても壊れないように書いてください（`if not exists` など）。

## テスト

```bash
npm test        # 版番号の並び順・対象ファイルの切り分けを確認する
```
...
- **アカウントを作れるメールドメインは限定してある。** `schema_v15_signup_allowlist.sql` の
  `public.signup_allowed_domains()` に無いドメインは、`handle_new_user()` が例外を投げて
  登録そのものを拒否する。ダッシュボードの Enable Sign Ups はオンのままでよい
  （初回の Google ログインは新規アカウント作成として扱われるため、オフにすると
  まだ一度もログインしていない社員が締め出される）。
  - 許可ドメインを増やすときは、この関数を `create or replace` する .sql を新しく足すこと
    （**一覧はこの関数1か所だけ**。既存ファイルは書き換えない）。
  - 社外向けデモ用に許可ドメイン外のアドレスを**新しく**発行したいときは、先にそのドメインを
    上の一覧に足しておく。**すでに `public.users` に行があるアカウントは対象外**なので、
    運用中のデモアカウントはそのままログインできる。
  - フロント側（`src/login.js`）にドメイン判定は置かない。APIを直接叩けば迂回されるため。
- ログイン済みユーザーの権限
  - 日報の**閲覧**は全員分
  - **作成・更新・削除**は自分の日報のみ

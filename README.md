...
- **Email の Enable Sign Ups は必ずオフにする** … オンだと、公開JSに必ず含まれる匿名キーで
  サインアップAPIを直接叩かれ、誰でもアカウントを作れてしまう。上のとおり社外向けデモ用の
  アカウントはダッシュボードから手で発行する運用なので、オフでも困らない
...
- **アカウントを作れるメールドメインは限定してある。** `schema_v15_signup_allowlist.sql` の
  ...
  登録そのものを拒否する。ダッシュボードの設定に頼らず、DB側でも閉じている。
